/**
 * ScraperActivity — observability dashboard for Go scraper batches.
 * Reads from /scraper/runs.
 */
...
export default function ScraperActivity() {
  ...
  const { data: runs = [], isLoading, refetch } = useQuery({
    ...
    queryFn: () => scraperApi.listRuns(50),
  ...
  const runScraperMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: 'Posts will appear in the queue within 1–2 minutes.',
      });
      setTimeout(() => queryClient.invalidateQueries({ queryKey: ['scraper-runs'] }), 3000);
    },
    onError: (e) => toastApiError(e, 'Failed to trigger scraper'),
  });

  const totalItems = runs.reduce((acc, r) => acc + (r.total_items_pulled || 0), 0);
  const totalNew = runs.reduce((acc, r) => acc + (r.new_items_inserted || 0), 0);
  const totalDup = runs.reduce((acc, r) => acc + (r.duplicate_items_skipped || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Scraper Activity</h2>
          <p className="text-sm text-muted-foreground mt-0.5">
            Every Go scraper batch is logged here for end-to-end debugging.
          </p>
        </div>
        <Button
          variant="gradient"
          ...
          {isLoading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                The next scheduled batch runs within 30 minutes — or hit "Run scraper now".
              </p>
            </div>
          ) : (
